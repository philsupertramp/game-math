...
std::pair<MatrixDS<N, OutputCount, double>, MatrixDS<N, OutputCount, double>>
feedForward(const MatrixDS<N, InputCount, double> &inputs, const MatrixDS<InputCount+1, OutputCount, double> &weights, const MatrixDS<N, 1, double> &biases);
        ...
        void ReadFromFile(const char* fileName){
            ...
            if (dataFile.is_open())
            {
                while ( getline (dataFile,line) )
                {
                    ...
                    std::string val;
                    std::getline(dataFile, val, '\t'); Inputs[count][0] = std::atof(val.c_str()); finishedPart += val + "\t";
                    std::getline(dataFile, val, '\t'); Inputs[count][1] = std::atof(val.c_str()); finishedPart += val + "\t";
                    std::getline(dataFile, val, '\t'); Inputs[count][2] = std::atof(val.c_str()); finishedPart += val + "\t";
                    std::getline(dataFile, val, '\t'); Inputs[count][3] = std::atof(val.c_str()); finishedPart += val + "\t";
                    std::getline(dataFile, val, '\t'); Outputs[count][0] = std::atoi(val.c_str()); finishedPart += val + "\t";
                    std::getline(dataFile, val, '\t'); Outputs[count][1] = std::atoi(val.c_str()); finishedPart += val + "\t";
                    val = line; Outputs[count][2] = std::atoi(val.c_str());
...
public:
    ...
    MatrixDS<InputCount+1,OutputCount,T> BackPropagate([[maybe_unused]] const DataSet<N> &ds, const MatrixDS<InputCount+1, OutputCount, T> &weight, [[maybe_unused]] const double eta){
        auto sampleCount = N;
    ...
    void Train(const size_t maxIterations = 500, const double maxWeight = 0.5, const double eta = 0.1){
        if(trainingSet == nullptr || validationSet == nullptr || testSet == nullptr){
            return;
        }
...
//        const size_t maxEpoch = 5;
        ...
        std::vector<double> x3; std::vector<double> y3;
        for(size_t i = 0; i <= maxIterations; i++){
            x1.push_back(trainingErrors[i][0][0]); y1.push_back(trainingErrors[i][1][0]);
            x2.push_back(trainingErrors[i][0][1]); y2.push_back(trainingErrors[i][1][1]);
            x3.push_back(trainingErrors[i][0][2]); y3.push_back(trainingErrors[i][1][2]);
        }
        ...
        printVec(x1); printVec(y1);
        printVec(x2); printVec(y2);
        printVec(x3); printVec(y3);
    }
...
void InitializeWeights(MatrixDS<N,M,double> &weights, const double &minWeight, const double &maxWeight){
    ...
    for(size_t i = 0; i < N; ++i) {
        for(size_t j = 0; j < M; ++j) {
            weights[i][j] = (double) minWeight + ((rand() % 100) * diff/100.0);
        }
    }
}
...
template<size_t N, size_t InputCount, size_t OutputCount>
std::pair<MatrixDS<N, OutputCount, double>, MatrixDS<N, OutputCount, double>> feedForward(const MatrixDS<N, InputCount, double> &inputs, const MatrixDS<InputCount+1, OutputCount, double> &weights, const MatrixDS<N, 1, double> &biases){
