...
public:
    ...
    : PolynomialBase(X, Y)
    {
         ...
         */
        if(out.Determinant() == 1.0){
            std::cerr << "Error computing Vandermonde-Matrix!" << std::endl;
            return;
        }
        A = gaussSeidel(out, Y);
    }
...
public:
    ...
    [[nodiscard]] std::string buildLx(size_t i) const {
      ...
      for(size_t j = 0; j < x.rows(); ++j){
          if(i == j) { needsOperator = false; continue;}
...
public:
    ...
    virtual Matrix<double> Evaluate(const Matrix<double>& xIn) const override {
        ...
        for(int j = b.rows()-2; j >= 0; --j) {
            auto bAct = b(j, 0);
            auto xAct = X(j, 0);
            y = xIn.Apply([xAct](const double& in){return in - xAct; }) // (xIn - X[j])
