 ...
 * @return SVD of A
 */
std::vector<Matrix<double>, Matrix<double>, Matrix<double>> svd(const Matrix<double>& A, const size_t& k, const double epsilon = 0.1e-4){
     ...
     auto Q = Matrix<double>::Random(n, k);
     auto res = qr(Q);
     Q = res.first;
     auto Q_prev = Q;
     ...
     auto singular_values = diag(R).Apply([](double val){return sqrt(val); });
