...
class QRTestCase : public Test
{
    ...
    bool TestPCANormalDistribution(){
        ...
        for(size_t i = 0; i < X.rows(); ++i){
            row_mean(0, i) = mean(X.GetSlice(i, i, 0, X.columns() - 1));
        }
        ...
        auto newTheta = HorizontalConcat(Theta.Apply([](double val){ return cos(val); }), Theta.Apply([](double val){ return sin(val);})).Transpose();
        ...
        Matrix<double> pc2 = {
            {mean1, mean2},
            {mean1 + U(0, 1) * S(0, 1), mean2 + U(1, 1) * S(0, 1)}
        };
        std::cout << "PC2:\n" << pc2 << std::endl;
        plot.AddData(
            pc2,
